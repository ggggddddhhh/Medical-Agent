...
export const LINGUISTIC_ASSERTION_LAYER_VERSION = "linguistic-assertion-layer-0.1.0";

const CORRECTION = /更正|纠正|说错了|说错|改口|重新想了想|重新想想|记错了|记错|其实|准确地说/;
const CERTAIN = /确实|真的|肯定|其实|明确|就是/;
const UNCERTAIN = /好像|可能|也许|似乎|不太确定|不确定|说不准|记不清|拿不准|觉得|像是/;
const HYPOTHETICAL = /如果|假如|要是|万一|假设|倘若/;
const SOURCE = /网上|文章|资料|举例|例如|比如|听说|写着|别人说|医生问/;
const OTHER_PERSON = /我朋友|朋友|我家人|家里人|我爱人|爱人|同事|别人|他|她/;
const NEGATION = /没有|并无|无明显|否认|不伴|从未|不是|并非|没(?:有)?|不(?:会|曾|再|是|伴|往|向|硬|疼|痛)/;
const STRONG_PAST = /昨天|昨晚|上周|上星期|以前|曾经|之前|早些时候|当时|那次|过去/;
const RECENT_PAST = /刚才|刚刚/;
const CURRENT = /现在|正在|目前|仍然|还在|今天|此刻/;
const RESOLVED = /已经.{0,8}(?:好了|缓解|消失|不疼|不痛)|现在.{0,8}(?:没有|没再|不再|正常|清醒)/;
const NEW_ONSET = /突然|一下子?|瞬间|猛地|骤然|几秒|十来秒|十几[秒妙]|半分钟|一分(?:钟)?内/;
...
function classify(message, span) {
  ...
  const patientObservedByOther = /(?:家人|朋友|同事).{0,8}说.{0,20}(?:我|叫不醒我)/.test(nearby);
  ...
  const certainty = CERTAIN.test(clause.text) || CORRECTION.test(clause.text)
    ? "certain"
    : UNCERTAIN.test(clause.text) || UNCERTAIN.test(nearby)
      ? "uncertain"
      : "certain";
  return {
    ...
    polarity,
    certainty,
    temporality: classifyTemporality(clause.text, nearby),
    quote,
    hypothetical,
...
function classifyTemporality(clause, nearby) {
  if (RESOLVED.test(clause)) return "resolved";
  if (STRONG_PAST.test(clause)) return "previous";
  if (NEW_ONSET.test(clause) || /头[痛疼]后/.test(clause)) return "new_onset";
  if (
    RECENT_PAST.test(clause) ||
    /眼前一黑|栽倒|倒下|昏过去|晕了一下|腿一软|窜到了|胸(?:口)?[痛疼]时|头[痛疼]时/.test(clause)
  ) return "previous";
